...
export const AnnouncementsView: React.FC<AnnouncementsViewProps> = ({ courseId }) => {
  ...
  const [sectionRestriction, setSectionRestriction] = useState('All Sections');
  const [delayPosting, setDelayPosting] = useState(false);
  const [delayedDate, setDelayedDate] = useState('');
  const [allowComments, setAllowComments] = useState(true);
  const [usersMustPostBeforeReplies, setUsersMustPostBeforeReplies] = useState(false);
  ...
  const handleCreateSubmit = (e: React.FormEvent) => {
    ...
    const attachments = attachmentName.trim()
      ? [{ name: attachmentName.trim(), size: '1.4 MB' }]
      : [];
    ...
    showAlert({
      ...
      message: `Announcement "${title.trim()}" has been posted successfully to ${sectionRestriction}.`,
      type: 'success'
    });
  ...
  const canCreate = activeRole === 'faculty' || activeRole === 'admin';
    ...
    <div className="space-y-6">
      ...
      {isComposeOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-card border border-border rounded-2xl w-full max-w-2xl shadow-elevated overflow-hidden animate-scale-in my-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-border">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-background border border-border hover:border-primary/40 focus:border-primary focus:ring-2 focus:ring-primary/20 rounded-xl text-xs font-sans text-foreground outline-none shadow-subtle cursor-pointer transition-all"
                  >
                    <option value="All Sections">All Sections</option>
                    <option value="BSCS 4-1">BSCS 4-1</option>
                    <option value="BSCS 4-2">BSCS 4-2</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Paperclip className="w-3.5 h-3.5 text-muted-foreground absolute left-3 top-3" />
                    <input
                      type="text"
                      value={attachmentName}
                      onChange={e => setAttachmentName(e.target.value)}
                      placeholder="e.g. Midterm_Guidelines.pdf"
                      className="w-full pl-9 pr-3 py-2 bg-background border border-border rounded-xl text-xs font-sans text-foreground outline-none"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-muted/40 rounded-xl border border-border space-y-2.5 text-xs">
                ...
                {allowComments && (
                  <label className="flex items-center space-x-2.5 cursor-pointer pl-6">
                    <input
                      type="checkbox"
                      checked={usersMustPostBeforeReplies}
                      onChange={e => setUsersMustPostBeforeReplies(e.target.checked)}
                      className="w-4 h-4 accent-primary rounded"
                    />
                    <span className="text-muted-foreground">Users must post before seeing replies</span>
                  </label>
                )}
        ...
        <div className="space-y-4">
          {filteredAnnouncements.map(ann => {
            ...
            return (
              <div
                key={ann.id}
                className={`bg-card border rounded-2xl transition-all shadow-subtle overflow-hidden ${
                  ann.pinned
                  ...
                  <div className="p-5 pt-0 border-t border-border/80 space-y-5 bg-card/60">
                    ...
                    {ann.attachments && ann.attachments.length > 0 && (
                      <div className="space-y-1.5 pt-2">
                        <span className="text-[11px] font-sans font-bold uppercase text-muted-foreground">
                          Attached Materials
                        </span>
                        <div className="flex flex-wrap gap-2">
                          {ann.attachments.map((file, idx) => (
                            <div
                              key={idx}
                              className="px-3 py-1.5 bg-muted/60 border border-border rounded-xl flex items-center space-x-2 text-xs font-sans text-foreground"
                            >
                        ...
                        <div className="flex items-start space-x-2 pt-2">
                          ...
                          <div className="flex-1 space-y-2">
                            <textarea
                              ...
                              placeholder="Type a response or inquiry..."
