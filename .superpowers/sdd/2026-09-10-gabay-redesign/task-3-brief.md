# Task 3 brief — LMSContextPanel (Level-2)
...
**Interfaces:**
- Consumes: `LMS_CHILDREN, COURSE_CHILDREN, isVisible` from `src/config/navigation.ts`; `useLMS()` (`db, activeCourseId, activeRole, activeUser`).
...
Note: `onNavigateCourse` is accepted in props for Task 4 wiring but unused in this component's render — keep it in the signature verbatim (no unused-param lint failure: oxlint only, tsc `noUnusedParameters` not enabled for destructured props object `p`).

Steps:
- [ ] Step 1: confirm file absent.
- [ ] Step 2: baseline typecheck.
- [ ] Step 3: create verbatim. No other files.
- [ ] Step 4: typecheck PASS + build PASS.
- [ ] Step 5: `git add src/components/layout/LMSContextPanel.tsx` + `git commit -m "feat: add LMS context panel level-2"`.
